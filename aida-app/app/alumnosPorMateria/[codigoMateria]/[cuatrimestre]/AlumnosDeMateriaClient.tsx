...
import TablaAlumnosDeMateria from "@/components/tablaAlumnosDeMateria";
...
export default function AlumnosDeMateriaClient({
  ...
  error: string | null;
    ...
    const [lista, setLista] = useState<{lu: string; nombres: string; apellido: string; nota: number}[]>(alumnos);
    ...
    const ponerNotaAAlumno = async (
        ...
        try {
        const res = await apiClient("/materias/update", {
            method: "POST",
    ...
    if (actionError)
    return <ErrorScreen error={actionError}/>
