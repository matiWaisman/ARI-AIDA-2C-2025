...
export async function getCertificadoLUFromBackend(
  req: Request,
  res: Response
): Promise<void> {
  try {
    const response = await fetch(
      `http://localhost:3000/app/lu?LU=${req.params.lu}`
    );
    ...
    res.status(500).send(`
            <html>
                <body>
                    <h1>Error</h1>
                    <p>No se pudo obtener el certificado para el LU: ${
                      req.params.lu
                    }</p>
                    <p>Error: ${
...
export async function getCertificadoFechaFromBackend(
  req: Request,
  res: Response
): Promise<void> {
  try {
    const response = await fetch(
      `http://localhost:3000/app/fecha?Fecha=${req.params.fecha}`
    );
    ...
    res.status(500).send(`
            <html>
                <body>
                    <h1>Error</h1>
                    <p>No se pudo obtener el certificado para la fecha: ${
                      req.params.fecha
                    }</p>
                    <p>Error: ${
                      error instanceof Error ? error.message : String(error)
...
export async function postCargarCSVFromBackend(
  req: Request,
  res: Response
): Promise<void> {
  try {
    ...
    const response = await fetch(
      `http://localhost:3000/app/cargarCSV?FilePath=${encodeURIComponent(
        absolutePath
      )}`,
      { method: "POST" }
    );
