...
export function genericController(tableDef: TableDef) {
  ...
  const allParams = (params: Record<string, any>) =>
    allColnames.map((col) => params[col]);
  ...
  // Busca una fila por la pk completa
  ...
  // Busca una fila por una columna específica, no necesariamente la pk
  const getBy = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      if (!exists) {
        res
          .status(400)
          .json({ error: `Columna ${column} no existe en ${tableDef.name}` });
        return;
      }
  ...
  const deleteRow = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      const childTables = tableDefs
        .filter((t) => t.fks.some((fk) => fk.referencesTable === tableDef.name))
        .map((t) => ({
          table: t.name,
